  ...
  var body: some View {
    GeometryReader { geometry in
      ...
      let polaroidSize = orientation.isLandscape
        ? CGSize(
          width: calculatedFrameSize.width + thinBorder + thickBorder,
          height: calculatedFrameSize.height + thinBorder * 2
        )
        : CGSize(
          width: calculatedFrameSize.width + thinBorder * 2,
      ...
      ZStack {
        ...
        Image(decorative: image, scale: 1.0)
          ...
          .offset(
            x: offset.width + visualOffset.width,
...
struct PolaroidFrameOverlay: View {
  ...
  private var cutoutOffset: CGSize {
    ...
    case .landscape:
      CGSize(width: (thickBorder - thinBorder) / 2, height: 0)
    case .portraitFlipped:
      CGSize(width: 0, height: (thickBorder - thinBorder) / 2)
    case .landscapeFlipped:
      CGSize(width: -(thickBorder - thinBorder) / 2, height: 0)
